...
{
    if (val & 1)
        printf("\nAICA transfer sys_addr:%08x g2_addr:%08x len:%d endrestart:%d", this->g2.SB_ADSTAR, this->g2.SB_ADSTAG, this->g2.SB_ADLEN.transfer_len, this->g2.SB_ADLEN.end_restart);
}

void G2_write_E1ST(struct DC* this, u64 val)
{
    if (val & 1)
        printf("\nEXT1 transfer sys_addr:%08x g2_addr:%08x len:%d endrestart:%d", this->g2.SB_E1STAR, this->g2.SB_E1STAG, this->g2.SB_E1LEN.transfer_len, this->g2.SB_E1LEN.end_restart);
}

void G2_write_E2ST(struct DC* this, u64 val)
{
    if (val & 1)
        printf("\nEXT2 transfer sys_addr:%08x g2_addr:%08x len:%d endrestart:%d", this->g2.SB_E2STAR, this->g2.SB_E2STAG, this->g2.SB_E2LEN.transfer_len, this->g2.SB_E2LEN.end_restart);
}
