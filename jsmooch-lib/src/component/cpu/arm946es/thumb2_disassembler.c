...
void ARM946ES_thumb_disassemble(u16 opc, jsm_string *out, i64 ins_addr, ARMctxt *ct)
{
    ...
    if ((opc & 0b1111100000000000) == 0b0001100000000000) { // ADD_SUB
    ...
    else if ((opc & 0b1111111100000000) == 0b0100011100000000) { // BX, BLX
        dasm_BX_BLX(opc, out, ins_addr, ct);
    }
    ...
    else if ((opc & 0b1111111100000000) == 0b1011111000000000) // 10111110..  BKPT
        dasm_BKPT(opc, out, ins_addr, ct);
        ...
        dasm_BLX_suffix(opc, out, ins_addr, ct);
