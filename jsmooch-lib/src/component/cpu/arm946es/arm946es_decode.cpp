...
#define OPCd(n,a,b) if ((opc & a) == b) return IK::n

static IK::ins_kind decode_arm(u32 opc)
{
...
static IK::ins_kind decode_arm_never(u32 opc)
{
...
void core::fill_arm_table()
{
    for (u32 opc = 0; opc < 4096; opc++) {
        arm9_ins *ins = &this->opcode_table_arm[opc];
        IK::ins_kind k = decode_arm(opc);
        switch(k) {
#define I(x) case IK::x: ins->exec = &core::ins_##x; break;
            I(MUL_MLA)
            I(MULL_MLAL)
            I(SWP)
            I(LDRH_STRH)
...
#undef I
#define I(x) case IK::x: ins->exec = &core::ins_##x; ins->valid = true; break;
...
#define BITS(hi,lo) (doBITS(opc, hi, lo))
    ins->opcode = opc;
    ins->func = &core::THUMB_ins_INVALID;
    if ((opc & 0b1111100000000000) == 0b0001100000000000) { // ADD_SUB
        ins->func = &core::THUMB_ins_ADD_SUB;
        ...
        ins->func = &core::THUMB_ins_LSL_LSR_ASR;
        ...
        ins->func = &core::THUMB_ins_ADD_CMP_MOV_hi;
        ins->Rd = OBIT(7) << 3;
        ins->Rs = OBIT(6) << 3;
        ins->Rs |= BITS(5,3);
        ins->Rd |= BITS(2, 0);
        ...
        ins->func = &core::THUMB_ins_LDR_PC_relative;
        ins->Rd = BITS(10, 8);
        ins->imm = BITS(7, 0);
        ins->imm <<= 2;
    }
    else if ((opc & 0b1111011000000000) == 0b0101001000000000) { // 0101.01...
        ins->func = &core::THUMB_ins_LDRH_STRH_reg_offset;
        ins->L = OBIT(11);
        ins->Ro = BITS(8, 6);
        ins->Rb = BITS(5, 3);
        ins->Rd = BITS(2, 0);
        ...
        ins->func = &core::THUMB_ins_LDRSH_LDRSB_reg_offset;
        ...
        ins->func = &core::THUMB_ins_LDR_STR_reg_offset;
        ins->L = OBIT(11);
        ins->Ro = BITS(8, 6);
        ins->Rb = BITS(5, 3);
        ins->Rd = BITS(2, 0);
        ...
        ins->func = &core::THUMB_ins_LDRB_STRB_reg_offset;
        ...
        ins->func = &core::THUMB_ins_LDR_STR_imm_offset;
        ins->L = OBIT(11);
        ins->imm = BITS(10, 6);
        ins->imm <<= 2;
        ins->Rb = BITS(5, 3);
        ...
        ins->func = &core::THUMB_ins_LDRB_STRB_imm_offset;
        ins->L = OBIT(11);
        ins->imm = BITS(10, 6);
        ins->Rb = BITS(5, 3);
        ins->Rd = BITS(2, 0);
        ...
        ins->func = &core::THUMB_ins_LDRH_STRH_imm_offset;
        ins->L = OBIT(11);
        ins->imm = BITS(10, 6);
        ins->imm <<= 1;
        ins->Rb = BITS(5, 3);
        ...
        ins->func = &core::THUMB_ins_LDR_STR_SP_relative;
        ins->L = OBIT(11);
        ins->Rd = BITS(10, 8);
        ins->imm = BITS(7, 0);
        ins->imm <<= 2;
        ...
        ins->func = &core::THUMB_ins_ADD_SP_or_PC;
        ins->SP = OBIT(11);
        ins->Rd = BITS(10, 8);
        ins->imm = BITS(7, 0);
        ins->imm <<= 2;
