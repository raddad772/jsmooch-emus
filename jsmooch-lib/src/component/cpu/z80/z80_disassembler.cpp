...
void disassemble_entry(core *th, disassembly_entry& entry)
{
    ...
    u32 PC = entry.addr;
    u32 IR = th->trace.strct.read_trace(th->trace.strct.ptr, PC);
    disassemble(PC, IR, th->trace.strct, entry.dasm);
...
u32 disassemble(u32 &PC, u32 IR, jsm_debug_read_trace &rt, jsm_string &out)
{
    ...
    PC = (PC + 1) & 0xFFFF;
    if (IR == S_DECODE) {
        return out.sprintf("DECODE");
    }
    else if (IR == S_RESET) {
        return out.sprintf("RESET");
    }
    else if (IR == S_IRQ) {
        return out.sprintf("IRQ");
    }
    ...
    while(decoded_bytes < 16) {
        ...
        switch(current_prefix) {
                    ...
                    case 0: // counter = 0
                        switch (z) {
                            case 0: // counter=0 z=0
                                switch (y) {
                                    case 0:
                                        ...
                                        break;
                                    case 1:
                                        l += out.sprintf("EX AF, AF'");
                                        break;
                                    case 2:
                                        l += out.sprintf("DJNZa %04x", sread8(PC, rt));
                                        break;
                                    case 3:
                                        l += out.sprintf("JR %04x", sread8(PC, rt));
                                        break;
                                    case 4:
                                    case 5:
                                    case 6:
                                    case 7:
                                        l += out.sprintf("JR %s, %04x", Z80D_tabl_cc[y - 4], sread8(PC, rt));
                                        break;
                                }
                                break;
                            ...
                            case 2: // counter = 0, z = 2
                                switch (p) {
                                    case 0:
                                        if (q == 0) l += out.sprintf("LD (BC), A");
                                        else l += out.sprintf("LD A, (BC)");
                                        break;
                                    case 1:
                                        if (q == 0) l += out.sprintf("LD (DE), A");
                                        else l += out.sprintf("LD A, (DE)");
                                        break;
                                    case 2:
                                        if (q == 0) {
                                            l += out.sprintf("LD (%04x), ", read16(PC, rt));
                                            w += l;
                                        ...
                                        break;
                                    case 3:
                                        if (q == 0) l += out.sprintf("LD (%04x), A", read16(PC, rt));
                                        else l += out.sprintf("LD A, (%04x)", read16(PC, rt));
                                        break;
                                }
                                break;
                            case 3: // counter = 0, z = 3
                                if (q == 0) l += out.sprintf("INC ");
                                else l += out.sprintf("DEC ");
                            ...
                            case 6: {
                                ...
                                l += a;
                                w += a;
                                l += out.sprintf(", %02x", read8(PC, rt));
                                break; }
                            case 7: {
                                const char *att[] = {"RLCA", "RRCA", "RLA", "RRA", "DAA", "CPL", "SCF", "CCF"};
                                l += out.sprintf("%s", att[y]);
                                break;
                            }
                        }
                        break;
                    case 1: // counter = 1
                        if ((z == 6) && (y == 6)) l += out.sprintf("HALT");
                        else {
                            l += out.sprintf("LD ");
                            w += 3;
                            u32 a = repl0(PC, HL, H, L, rt, Z80D_tabl_r[y], w, sz - l);
                        ...
                        break;
                    case 2: {// counter = 2
                        u32 a = out.sprintf("%s ", Z80D_tabl_alu[y]);
                        l += a;
                        w += a;
                        l += repl0(PC, HL, H, L, rt, Z80D_tabl_r[z], w, sz - l);
                        break;
                    ...
                    case 3: // counter = 3
                        switch (z) {
                            case 0: // counter=3 z=0
                                l += out.sprintf("RET %s", Z80D_tabl_cc[y]);
                                break;
                            case 1: // counter=3 z=1
                                ...
                                break;
                            case 2:
                                l += out.sprintf("JP %s, %04x", Z80D_tabl_cc[y], read16(PC, rt));
                                break;
                            case 3: // counter=3 z=3
                                switch (y) {
                                    case 0:
                                        l += out.sprintf("JP %04x", read16(PC, rt));
                                        break;
                                    case 1:
                                        l += out.sprintf("INVALID1");
                                        break;
                                    case 2:
                                        l += out.sprintf("OUT (%02x), A", read8(PC, rt));
                                        break;
                                    case 3:
                                        l += out.sprintf("IN (%02X), A", read8(PC, rt));
                                        break;
                                    case 4:
                                        l += out.sprintf("EX (SP), ");
                                        w += 9;
                            ...
                            case 4: // counter=3 z=4
                                l += out.sprintf("CALL %s, %04x", Z80D_tabl_cc[y], read16(PC, rt));
                                break;
                            case 5: // counter=3 z=5
                                if (q == 0) {
                                    ...
                                    l += out.sprintf("CALL %04x", read16(PC, rt));
                                break;
                            case 6: // counter=3 z=6
                                l += out.sprintf("%s %02x", Z80D_tabl_alu[y], read8(PC, rt));
                                break;
                            case 7: // counter=3 z=7
                                l += out.sprintf("RST %02x", y * 8);
                                break;
                        }
                    default: ;
                }
            ...
            case 0xCB: // prefix 0xCB
                switch(x) {
                    case 0:
                        l += out.sprintf("%s %s", Z80D_tabl_rot[y], Z80D_tabl_r[z]);
                        break;
                    case 1:
                        l += out.sprintf("TEST %d, %s", y, Z80D_tabl_r[z]);
                        break;
                    case 2:
                        l += out.sprintf("RES %d, %s", y, Z80D_tabl_r[z]);
                        break;
                    case 3:
                        l += out.sprintf("SET %d, %s", y, Z80D_tabl_r[z]);
                        break;
                }
                break;
            case 0xED:
                switch(x) {
                    case 0:
                    case 3:
                        l += out.sprintf("INVALID NONI NOP");
                        break;
                    case 1: // 0xED counter=1
                        switch(z) {
                            case 0: // 0xED counter=1 z=0
                                if (y == 6) l += out.sprintf("IN (C)");
                                else l += out.sprintf("IN %s, (C)", Z80D_tabl_r[y]);
                                break;
                            case 1:
                                if (y == 6) l += out.sprintf("OUT (C)");
                                else l += out.sprintf("OUT %s, (C)", Z80D_tabl_r[y]);
                                break;
                            case 2: // 0xED counter=1 z=2
                                if (q == 0) l += out.sprintf("SBC HL, %s", Z80D_tabl_rp[p]);
                                else l += out.sprintf("ADC HL, %s", Z80D_tabl_rp[p]);
                                break;
                            case 3: // 0xED counter=1 z=3
                                if (q == 0) l += out.sprintf("LD (%04x), %s", read16(PC, rt), Z80D_tabl_rp[p]);
                                else l += out.sprintf("LD %s, (%04x)", Z80D_tabl_rp[p], read16(PC, rt));
                                break;
                            case 4:
                                l += out.sprintf("NEG");
                                break;
                            case 5:
                                if (y == 1) l += out.sprintf("RETI");
                                else l += out.sprintf("RETN");
                                break;
                            case 6: // 0xED counter=1 z=6
                                l += out.sprintf("IM %s", Z80D_tabl_im[y]);
                                break;
                            case 7: // 0xED counter=1 z=7
                                l += out.sprintf("%s", (char[8][10]){"LD I, A", "LD R, A", "LD A, I", "LD A, R", "RRD", "RLD", "NOP", "NOP"}[y]);
                                break;
                        }
                        break;
                    case 2: // 0xED counter=2
                        if ((z <= 3) && (y >= 4)) {
                            l += out.sprintf("%s", Z80D_tabl_bli[y][z]);
                        }
                        else
                            l += out.sprintf("INVALID#2 NONI NOP");
                        break;
                }
                break;
            case 0xDDCB:
            case 0xFDCB:
                ...
                switch(x) {
                    case 0: // CBd counter=0
                        if (z != 6) l += out.sprintf("LD %s, %s(%s+%s)", Z80D_tabl_r[z], Z80D_tabl_rot[y], IXY, d);
                        else l += out.sprintf("%s (%s+%s)", Z80D_tabl_rot[y], IXY, d);
                        break;
                    case 1: // CBd counter=1
                        l += out.sprintf("TEST %d, (%s+%s)", y, IXY, d);
                        break;
                    case 2: // CBd counter=2
                        if (z != 6)
                            l += out.sprintf("LD %s, RES %d, (%s+%s)", Z80D_tabl_r[z], y, IXY, d);
                        else
                            l += out.sprintf("RES %d, (%s+%s)", y, IXY, d);
                        break;
                    case 3: // CBd counter=3
                        if (z != 6)
                            l += out.sprintf("LD %s, SET %d, (%s+%s)", Z80D_tabl_r[z], y, IXY, d);
                        else
                            l += out.sprintf("SET %d, (%s+%s)", y, IXY, d);
                        break;
                }
                break;
            default:
                l += out.sprintf("HOW DID WE GET HERE GOVNA");
                break;
        }
        if (l != 0) break;
        decoded_bytes++;
