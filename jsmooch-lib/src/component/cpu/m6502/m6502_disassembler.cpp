...
static void absolute(SARG)
{
    ...
    outstr.sprintf("%s$%04x", ins, v);
}

static void absolute_x(SARG) {
    u16 v = dbg_read16(trace, PC);
    outstr.sprintf("%s$%04x,x", ins, v);
};

static void absolute_y(SARG) {
    u16 v = dbg_read16(trace, PC);
    outstr.sprintf("%s$%04x,y", ins, v);
};

static void branch(SARG) {
    u16 v = (u16)(i8)dbg_read(trace, PC);
    u16 pc = v + (*PC);
    outstr.sprintf("%s$%04x", ins, pc);
}
...
{
    outstr.sprintf("%s($%04x)", ins, dbg_read16(trace, PC));
}

static void indirect_x(SARG)
{
    outstr.sprintf("%s($%04x,x)", ins, dbg_read16(trace, PC));
}

static void indirect_y(SARG) {
    outstr.sprintf("%s($%04x),y", ins, dbg_read16(trace, PC));
}
...
{
    outstr.sprintf("%s$%02x,x", ins, dbg_read(trace, PC));
}
...
void M6502_disassemble_entry(struct M6502 *cpu, struct disassembly_entry &entry)
{
