...
def C_func_sig(mo, tbit):
    return 'static void ' + C_func_name(mo, tbit) + '(struct HUC6280_regs *regs, HUC6280_pins *pins)'
...
def TransferAccumulatorToMPR(ag: huc6280_switchgen) -> str:
    ...
    ag.addl('    u32 mpl = regs->MPL << 13;')
    ag.addl('    for (u32 i = 0; i < 8; i++) {')
    ag.addl('        u32 shifted = 1 << i;')
    ag.addl('        if ((regs->TA & shifted) && (regs->MPR[i] != mpl)) {')
    ag.addl('            debugger_interface_dirty_mem(pins->debugger_interface, pins->debugger_mem_bus, mpl, ((i + 1) << 13) - 1);')
    ag.addl('            regs->MPR[i] = mpl;')
    ag.addl('        }')
    ag.addl('    }')
    ag.addl('}')
...
def main():
    ...
    with open(outinsfile, 'w') as outfile:
        ...
        outfile.write('\n')
        #outfile.write('static voidHUC6280_ins_NONE(struct HUC6280_regs *regs, HUC6280_pins *pins)\n')
        #outfile.write('{\n')
