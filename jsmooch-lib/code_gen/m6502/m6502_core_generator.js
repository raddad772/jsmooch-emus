...
class m6502_switchgen {
    ...
    BRK(vector='0xFFFE', set_b=true, add_one_to_pc=false) {
        ...
        this.addcycle();
        this.addl('regs.P.B = 1; // Confirmed via Visual6502 that this bit is actually set always during NMI, IRQ, and BRK. It basically always stays 1 unless forced to 0');
    ...
    ANC(what = 'regs.TR') {
        // AND #i, set N and Z. Copy N to C.
        this.AND(what);
        this.addl('regs.P.C = regs.P.N;');
    }
    ...
    add_ins(opcode_info, out_reg='regs.TR') {
        ...
        switch(ins) {
            case M6502_MN.ALR: /* ----------Unofficial opcodes */
                this.ALR(out_reg);
                break;
            case M6502_MN.ANC:
                this.ANC(out_reg);
            ...
            case M6502_MN.XAA:
                ...
                this.addl('pins.D = regs.A;');
                this.setn('regs.A');
                this.setz('regs.A');
                break;
            case M6502_MN.ADC: /* Official opcodes */
                this.ADC(out_reg);
                break;
            case M6502_MN.AND:
                this.AND(out_reg);
            ...
            case M6502_MN.SMB7:
                ...
                break;
            default:
                console.log('M6502 unhandled instruction ', ins, opcode_info);
                break;
        }
    }
...
function m6502_generate_instruction_function(indent, opcode_info, BCD_support=true, INVALID_OP='', final_variant, is_C=false) {
    ...
    switch(opcode_info.addr_mode) {
        ...
        case M6502_AM.X_INDr: // like ORA (oper,X) 6 cycles
            ...
            ag.add_ins(opcode_info, 'pins.D');
            break;
        case M6502_AM.X_INDm: // Like SLO (oper,X) 2 bytes 8 cycles
            ag.addcycle();
            ag.operand();
            ...
            ag.addcycle('spurious')
            ag.RW(1);
            ag.addcycle('write result')
            ag.add_ins(opcode_info, 'pins.D');
            break;
        ...
        case M6502_AM.IND_Yr: // Like LDA (oper), Y 5-6 cycles
            ...
            ag.add_ins(opcode_info, 'pins.D');
            break;
        case M6502_AM.IND_Ym: // Like SLO (oper), Y 8 cycles
            ag.addcycle('Get ZP'); // 1
            ag.operand();

            ag.addcycle('get ABS L'); // 2
            ag.addl('pins.Addr = pins.D;');

            ag.addcycle('get ABS H'); // 3
            ag.addl('regs.TR = pins.D;');
            ag.addl('regs.TA = pins.D + regs.Y;')
            ag.addr_inc_ZP();

            ag.addcycle('idle if crossed') // 4?
            // top of before, bottom of after (+Y)
            ag.addl('regs.TR = (regs.TR + (pins.D << 8)) & 0xFFFF;'); // before
            ag.addl('regs.TA = (regs.TA + (pins.D << 8)) & 0xFFFF;'); // after

            if (!CMOS) {
                ag.addl('if ((regs.TR & 0xFF00) ' + GENEQO + ' (regs.TA & 0xFF00)) { pins.Addr = regs.TA; break; }');
                ag.addl('pins.Addr = (regs.TR & 0xFF00) | (regs.TA & 0xFF);');
            }
            else {
                if (is_ADCSBA)
            ...
            ag.addcycle('duplicate write cycle');
            ag.add_ins(opcode_info, 'pins.D');
            ag.RW(1);
            break;
        case M6502_AM.IND_Yw: // Like STA (oper), Y  6 cycles
            ...
            ag.addl('regs.TR = regs.TA == pins.Addr;');
...
function get_decodes(opc_matrix, nonestr, func_prefix) {
    let out = '    ';
    let num_on_line = 0;
    let first_line = 1;
    for (let opc = 0; opc < 0x103; opc++) {
        if (num_on_line == 0) {
                out += '    ';
        } else {
            out += '  ';
        }
        if (!opc_matrix.hasOwnProperty(opc)) {
            out += nonestr;
        }
        else
            out += '&' + M6502_C_func_name(opc_matrix[opc], func_prefix);
...
function generate_6502_core_c(variant_list, final_variant, output_name, BCD_SUPPORT, INVALID_OP, is_C=true, func_prefix='') {
    ...
    let outstr = '#include <assert.h>\n' +
        ...
        '\n' +
        'static void ' + func_prefix + 'M6502_ins_NONE(struct M6502_regs *regs, M6502_pins *pins)\n' +
        '{\n' +
        '    assert(1==0);\n' +
        '}\n';
    let indent = '';
    ...
    for (let i in opc_matrix) {
        ...
        let mystr = M6502_C_func_signature(opc, func_prefix) + '\n{\n';
...
function generate_m6502_core_c(is_C=true) {
    return generate_6502_core_c([M6502_VARIANTS.STOCK, M6502_VARIANTS.STOCK_UNDOCUMENTED], M6502_VARIANTS.STOCK, 'm6502_opcodes_decoded', true, '', true, '');
}


function generate_nesm6502_core_c(is_C=true) {
    return generate_6502_core_c([M6502_VARIANTS.STOCK, M6502_VARIANTS.STOCK_UNDOCUMENTED], M6502_VARIANTS.STOCK, 'nesm6502_opcodes_decoded', false, '', true, 'nes');
}
...
/*    let outstr = '#include "helpers/int.h"\n' +
    ...
    let firstin = false;*/
    let opc_matrix = final_m6502_opcode_matrix(variant_list);
    let o = 'void M6502_ins_NONE(struct M6502_regs *regs, M6502_pins *pins);\n';
