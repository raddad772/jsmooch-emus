...
function replace_for_C(whatr) {
    ...
    whatr = whatr.replaceAll('cpu.regs.', 'YARYARYAR') // Reference to pointer

    whatr = whatr.replaceAll('regs.', 'cpu.regs.') // Reference to pointer
...
class SPC_funcgen {
    constructor(indent, opc_info) {
        ...
        this.outstr = '';
        this.addl1('// ' + opc_info.mnemonic + ' ' + opc_info.operand);
    ...
    BR(when, where) {
        ...
        this.addl(indent + 'regs.PC = (regs.PC + (u32)(i8)' + where + ') & 0xFFFF;');
    ...
    DIV() {
        ...
        this.addl('    regs.A = (YA / regs.X) & 0xFF;');
        this.addl('    regs.Y = (YA % regs.X) & 0xFF;')
        this.addl('} else {');
        this.addl('    regs.A = (255 - ((YA - (regs.X << 9)) / (256 - regs.X))) & 0xFF;');
    ...
    DOINS(ins, operand, operand2, addr_mode, opcode) {
        ...
        switch(ins) {
            ...
            case SPC_MN.RET1: // flag, PCL, PCH
                this.POP('regs.TR');
                this.addl('cpu.regs.P.setbyte(regs.TR);');
                this.POP16('regs.PC');
                break;
            case SPC_MN.POP:
                switch(opcode) {
    ...
    addl(what) {
        this.outstr += this.indent2 + replace_for_C(what) + '\n';
    }

    fetch_from_PC(who) {
        this.addl(who + ' = read8(cpu, regs.PC);');
    }
...
function SPC_func_name(opcode)
{
    let ostr = 'opcode_' + hex2(opcode);
    return ostr;
}

function SPC_generate_instruction_code_c(opcode)
{
    let outstr = '\n\nstatic void ' + SPC_func_name(opcode) + '(core &cpu) { ';
...
function SPC_generate_instruction_table(indent) {
    let ostr = '';
    let num = 0;
...
function SPC_decode_opcodes_c() {
    ...
    for (let j = 0; j < 0x100; j++) {
        funcstr += SPC_generate_instruction_code_c(j);
    }
    tablstr += SPC_generate_instruction_table('');
