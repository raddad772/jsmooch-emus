"use strict";
var WDC_PINS_SEPERATE_PDV = false;
...
class WDC_switchgen {
    ...
    SBC8() {
        ...
        this.addl('i32 A = regs.C & 0xFF; i32 data = (~regs.TR) & 0xFF;');
        this.addl('i32 result;')
    ...
    SBC16() {
        this.addl('i32 data = (~regs.TR) & 0xFFFF;');
        this.addl('i32 result;')
    ...
    SEPE() {
        ...
        this.addl('regs.P.v = regs.P.v | 0x30 | regs.TR;');
    }
    ...
    REPE(regs) {
        ...
        //Anded             = 1000
        //                    success!
        this.addl('regs.P.v = ((regs.P.v | 0x30) & (~(regs.TR | 0x30) & 0xFF));');
    }

    // Native Set bits from P
    SEPN() {
        this.addl('regs.P.v |= regs.TR;');
        this.addl('if (regs.P.X) { regs.X &= 0xFF, regs.Y &= 0xFF; }');
    }

    // Native Clear bits from P
    REPN() {
        this.addl('regs.P.v &= (~regs.TR & 0xFF);');
    }
    ...
    PULLP(E) {
        if (E)
            this.addl('regs.P.v = regs.TR | 0x30;');
        else
            this.addl('regs.P.v = regs.TR;');
    ...
    INTERRUPT(ins, E) {
        ...
        this.addr_to_S_then_dec();
        if (E && ins === WDC_OM.BRK)
            this.addl('pins.D = regs.P.v | 0x38;'); // Set BRK bit
        else if (E)
            this.addl('pins.D = regs.P.v & 0xF7;'); // Clear BRK bit
        else
            this.addl('pins.D = regs.P.v;');
...
function WDC_generate_instruction_function(indent, opcode_info, E, M, X) {
    ...
    switch(opcode_info.addr_mode) {
        ...
        case WDC_AM.PC_R:
            ...
            ag.addl('if (regs.TR) regs.PC = (regs.PC + ((u32)(i8)(regs.TA))) & 0xFFFF;');
            break;
        case WDC_AM.PC_RL:
            ag.addcycle(2);
            ag.RPDV(0, 1, 0, 0);
            ...
            ag.addcycle(4);
            ag.RPDV(0, 0, 0, 0);
            ag.addl('regs.TA = (u32)(i16)(regs.TA + (pins.D << 8));');
        ...
        case WDC_AM.STACKg: // RTI
            ...
            ag.addr_to_S_after_inc();
            if (E)
                ag.addl('regs.P.v = pins.D | 0b00110000;');
            else
                ag.addl('regs.P.v = pins.D;');
            ag.addl('if (regs.P.X) {');
            ag.addl('    regs.X &= 0xFF;');
            ag.addl('    regs.Y &= 0xFF;');
            ag.addl('}');
            ag.addl('if (regs.E) regs.S = (regs.S & 0xFF) | 0x100;')
    ...
    let outstr = '// ' + opcode_info.mnemonic + '\n' + ag.finished();
...
function EMX_table(E, M, X)
{
    if (E) return 4;
    return M + (X * 2);
}

function func_name(opcode, E, M, X)
{
    let opcode_info = WDC_opcode_matrix[opcode];
    let EMX = '';
    if (E) EMX = 'E';
    ...
    let ostr = 'opcode_' + hex2(opcode) + '_' + EMX;
    return ostr;
}
...
{
    let outstr = '\n\nstatic void ' + func_name(opcode, E, M, X) + '(regs &regs, pins &pins) { ';
    let opcode_info = WDC_opcode_matrix[opcode];
...
function decode_opcodes_c() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j <= WDC_MAX_OPCODE; j++) {
            funcstr += generate_instruction_code_c(j, IDT, E, M, X);
        }

        tablstr += generate_instruction_table(IDT, E, M, X, i === 4);
    }

    return outstr + funcstr + tablstr + '\n}\n';
}
