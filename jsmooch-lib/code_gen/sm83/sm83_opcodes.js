...
const INS_S_RESET = 0x101;
const INS_S_IRQ = 0x100;
...
const SM83_opcode_matrix = Object.freeze({
    ...
    [INS_S_IRQ]: new SM83_opcode_info(INS_S_IRQ, SM83_MN.S_IRQ),
    [INS_S_RESET]: new SM83_opcode_info(INS_S_RESET, SM83_MN.RESET)
});
...
function SM83_C_func_name(mo, CB=false) {
    if (CB)
        return 'ins_CB' + hex2(mo.opcode) + '_' + SM83_MN_R[mo.ins];
    else
        return 'ins_' + hex2(mo.opcode) + '_' + SM83_MN_R[mo.ins];
}

function SM83_C_func_signature(mo, CB=false) {
    return 'void ' + SM83_C_func_name(mo, CB) + '(regs &regs, pins &pins)'
}
...
function sm83_opcode_func_gen_c() {
    let o = '';
    let o2 = 'ins_func decoded_opcodes[0x202] = {\n';
